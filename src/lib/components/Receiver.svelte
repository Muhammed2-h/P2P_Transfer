  ...
  {#if state === TRANSFER_STATES.IDLE || state === TRANSFER_STATES.ERROR || state === TRANSFER_STATES.CONNECTING}
    ...
    {#if nearbyPeers.length > 0}
      <div class="nearby-section fade-in">
        <p class="nearby-label"><Wifi size={14} /> Nearby Devices</p>
        <div class="nearby-list">
          {#each nearbyPeers as peerCode}
            <button
              class="nearby-card"
              on:click={() => {
                code = peerCode;
            ...
            >
              ...
              <div class="nearby-info">
                <span class="nearby-name">Local Device</span>
                <span class="nearby-code">{peerCode}</span>
              </div>
            </button>
          {/each}
        </div>
            ...
            <div class="queue-card incoming-card">
              ...
              <div class="file-details">
                ...
                <div class="fmeta">
                  ...
                  >
                  {#if item.timestamp}
                    <span class="fdate">• {formatDate(item.timestamp)}</span>
                  {/if}
                </div>
              </div>
              <div class="action-area">
                {#if item.status === "queued"}
                  <button
                    class="btn-download"
                    on:click={() => requestDownload(item.id)}
                  >
                    <Download size={16} /> Download
                  </button>
                {:else if item.status === "transferring"}
                  <span class="status-text">Transferring...</span>
          ...
          {#each outgoing as item}
            <div class="queue-card outgoing-card">
              ...
              <div class="action-area">
                ...
                {:else if item.status === "completed"}
                  <span class="status-text text-success">Sent</span>
                {/if}
              </div>
            </div>
          {/each}
  ...
  {#if state === TRANSFER_STATES.TRANSFERRING || state === TRANSFER_STATES.PAUSED || state === TRANSFER_STATES.COMPLETED}
    <div class="transfer-status">
      <div class="status-header">
        <h3>
          {state === TRANSFER_STATES.COMPLETED ? "Received" : "Receiving"}: {$transfer.fileName}
        </h3>
